...
class StatesMutationDistribution(ListAPIView):
    ...
    def get_queryset(self):
        QuerySet = tsvfile.objects.values('state').annotate(
            Count('mutation_deletion', distinct=True))
...
class StatesSequenceDistribution(ListAPIView):
    ...
    def get_queryset(self):
        ...
        QuerySet = tsvfile.objects.filter(date__gte=days, year__in=year.split(',')).values(
            'state').annotate(Count('strain', distinct=True))
        return QuerySet
...
class StatesLineageDistribution(ListAPIView):
...
class Frequency(RetrieveAPIView):
    ...
    def get(self, request):
        ...
        df = df.groupby(["state", 'lineage', "mutation_deletion"]
...
class MonthlyDistribution(ListAPIView):
    ...
    def get_queryset(self):
        ...
        # QuerySet = [{'month_number': name, 'strain__count': value} for name, value in result.items()]
        print(QuerySet)
        return QuerySet
...
class WeeklyLineageStackBar(ListAPIView):
    ...
    def get(self, request):
        ...
        year = self.request.GET.get('year', "202")
        days = date.today()-timedelta(days=days)
        QuerySet = tsvfile.objects.filter(date__gte=days, month_number__icontains=year, lineage__in=[
                                          "AY.1", "BA.1"]).values('week_number', 'lineage').annotate(Count('strain', distinct=True))
        mnd = {}
        mnlist = []
        for dic in QuerySet:
            v = dic['week_number']
...
class StackBar1(ListAPIView):
    ...
    def get(self, request):
        ...
        days = date.today()-timedelta(days=days)
        QuerySet = tsvfile.objects.filter(date__gte=days, month_number__icontains=year).values(
            'month_number', 'lineage').annotate(Count('strain', distinct=True)).order_by('date__year', 'date__month')
        a = {}
...
class StackBar(ListAPIView):
    ...
    def get_queryset(self):
        ...
        QuerySet = tsvfile.objects.filter(date__gte=days, month_number__icontains=year).values(
            'month_number', 'lineage').annotate(Count('strain', distinct=True))
...
class GenomesSeqenced(ListAPIView):
    ...
    def get_queryset(self):
        ...
        QuerySet = tsvfile.objects.values('strain').distinct()
        return QuerySet
