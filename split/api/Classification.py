...
class LineageClassificationDistribution(ListAPIView):
    ...
    def get_queryset(self):
        ...
        QuerySet = tsvfile.objects.filter(date__gte=days,month_number__icontains=year).order_by().values('Class').annotate(Count('strain', distinct=True))
        return QuerySet
...
class MonthlyClassificationStackBar(ListAPIView):
    ...
    def get(self, request):
        ...
        year = self.request.GET.get('year',"202")
        days=date.today()-timedelta(days=days)
        QuerySet = tsvfile.objects.filter(date__gte=days,month_number__icontains=year).values('month_number','Class').annotate(Count('strain', distinct=True)).order_by('date__year','date__month')
        # QuerySet = QuerySet.filter(date__gte=days,month_number__icontains=year).values('month_number','Class').annotate(Count('strain', distinct=True)).order_by('date__year','date__month')
        mnd = {} ; mnlist = [] 
        for dic in QuerySet:
            v = dic['month_number']
            if v not in mnlist:
...
class WeeklyClassificationStackBar(ListAPIView):
    ...
    def get(self, request):
        ...
        year = self.request.GET.get('year',"202")
        days=date.today()-timedelta(days=days)
        QuerySet = tsvfile.objects.filter(date__gte=days,month_number__icontains=year).values('week_number','Class').annotate(Count('strain', distinct=True))
        mnd = {} ; mnlist = []
        for dic in QuerySet:
            v = dic['week_number']
            if v not in mnlist:
